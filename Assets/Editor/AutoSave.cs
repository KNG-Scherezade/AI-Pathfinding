...
public class AutoSave : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        intervalScene = EditorGUILayout.IntSlider("Interval (minutes)", intervalScene, 5, 10);
        if (isStarted)
        {
    ...
    void Update()
    {
        ...
        if (autoSaveScene)
        {
            if (DateTime.Now.Minute >= (lastSaveTimeScene.Minute + 5) || DateTime.Now.Minute == 59 && DateTime.Now.Second == 59)
            {
                saveScene();
            }
        }
